...
void DigiFont::drawSeg7(int seg, int x, int y, int c)
{
	...
	{
	case 0: // top
		for (i = 0; i < segThick; i++) (*linehFun)(x + (i < segSt ? segSt - i : i - segSt),
		                                           x + segWd - 1 - (i < segSt ? segSt - i : i - segSt), y + i,
		                                           c ? (i <= segSt ? colOn : colOn2) : colOff);
		break;
	case 3: // bottom
		for (i = 0; i < segThick; i++) (*linehFun)(x + (i < segSt ? segSt - i : i - segSt),
		                                           x + segWd - 1 - (i < segSt ? segSt - i : i - segSt),
		                                           y + segThick - 1 - i, c ? (i < segSt ? colOn2 : colOn) : colOff);
		break;
	case 6: // middle
	...
	case 1: // right
	case 2: // right
		for (i = 0; i < segThick; i++) (*linevFun)(x + segThick - 1 - i, y + (i < segSt ? segSt - i : i - segSt),
		                                           y + segHt - 1 - (i < segSt ? segSt - i : i - segSt),
		                                           c ? (i < segSt ? colOn2 : colOn) : colOff);
		break;
	case 4: // left
	case 5: // left
		for (i = 0; i < segThick; i++) (*linevFun)(x + i, y + (i < segSt ? segSt - i : i - segSt),
		                                           y + segHt - 1 - (i < segSt ? segSt - i : i - segSt),
		                                           c ? (i <= segSt ? colOn : colOn2) : colOff);
		break;
	}
...
int DigiFont::drawDigit1(int ch, int x, int y)
{
	...
	{
	case '.':
		(*rectFun)(x, y + segHt + segHt + 3 - segThick, segThick, segThick, colOn);
		return segThick;
	case ':':
...
int DigiFont::drawDigit2c(int ch, int x, int y)
{
	...
	case ':':
		...
		(*rectFun)(x, y + segHt + 1 + ofs + (segHt - segThick) / 2, segThick, segThick, colOn);
		(*rectFun)(x, y + segHt + 1 + ofs + (segHt - segThick) / 2 + segThick / 2, segThick, segThick - segThick / 2,
		           colOn2);
		return segThick;
	case ' ':
		s = 0;
...
int DigiFont::drawDigit3(int ch, int x, int y)
{
	...
	{
	case '.':
		(*rectFun)(x, y + digHt - 1 - segThick, segThick, segThick, colOn);
		return segThick;
	case ':':
...
int DigiFont::drawDigitO(int ch, int x, int y)
{
	...
	if (clearBg)
	{
		...
		(*linevFun)(x + segThick, y + segThick + hc + 1, y + segThick + hc + 1 + segThick - 2, colOff);
		// le-mid-short '3'
	...
	//(*linevFun)(x, y+segThick+hc+1,    y+segThick+hc+1+segThick-2, colOn); // le-mid-short full width
		(*linevFun)(x + digWd / 3 - 1, y + segThick + hc + 0, y + segThick + hc + 1 + segThick - 2, colOn);
	// le-mid-short 2/3 width
	...
	case 5:
		...
		break;
	case 6:
		rect(x + segThick - 1, y + hc + segThick * 2 - 1, digWd - segThick * 2 + 2, digHt - hc - segThick * 3 + 2,
		     colOn);
		(*linehFun)(x, x + digWd - 1, y, colOn); // top
		(*linehFun)(x + segThick, x + digWd - 1, y + segThick - 1, colOn); // top2
		(*linehFun)(x, x + digWd - 1, y + digHt - 1, colOn); // bottom
...
int DigiFont::numberWidth(char* txt)
{
	...
	while (*txt)
	{
		wd += ((*txt == '.' || *txt == ':') ? segThick : digWd) + spacing;
		txt++;
	}
	return wd - spacing;
}
